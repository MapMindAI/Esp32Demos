...
ERROR_t MCP2515_init() {
  ...
  if (MCP2515_Object->TXB_ptr == NULL || MCP2515_Object->RXB_ptr == NULL) {
    ESP_LOGE(TAG_MCP2515,
             "Couldn't initialize MCP2515_Object->(TXB_ptr || RXB_ptr). (NULL pointer)");
    return ERROR_FAIL;
  }
...
ERROR_t MCP2515_reset(void) {
  ...
  MCP2515_modifyRegister(MCP_RXB0CTRL, RXBnCTRL_RXM_MASK | RXB0CTRL_BUKT | RXB0CTRL_FILHIT_MASK,
                         RXBnCTRL_RXM_STDEXT | RXB0CTRL_BUKT | RXB0CTRL_FILHIT);
  MCP2515_modifyRegister(MCP_RXB1CTRL, RXBnCTRL_RXM_MASK | RXB1CTRL_FILHIT_MASK,
...
ERROR_t MCP2515_setConfigMode() { return MCP2515_setMode(CANCTRL_REQOP_CONFIG); }

ERROR_t MCP2515_setListenOnlyMode() { return MCP2515_setMode(CANCTRL_REQOP_LISTENONLY); }

ERROR_t MCP2515_setSleepMode() { return MCP2515_setMode(CANCTRL_REQOP_SLEEP); }

ERROR_t MCP2515_setLoopbackMode() { return MCP2515_setMode(CANCTRL_REQOP_LOOPBACK); }
...
ERROR_t MCP2515_setNormalMode() { return MCP2515_setMode(CANCTRL_REQOP_NORMAL); }
...
uint8_t MCP2515_getErrorFlags(void) { return MCP2515_readRegister(MCP_EFLG); }

void MCP2515_clearRXnOVRFlags(void) {
  MCP2515_modifyRegister(MCP_EFLG, EFLG_RX0OVR | EFLG_RX1OVR, 0);
}

uint8_t MCP2515_getInterrupts(void) { return MCP2515_readRegister(MCP_CANINTF); }

void MCP2515_clearInterrupts(void) { MCP2515_setRegister(MCP_CANINTF, 0); }

uint8_t MCP2515_getInterruptMask(void) { return MCP2515_readRegister(MCP_CANINTE); }
